...
public class AjioScraper implements EcommerceScraper {
    ...
    public List<Product> searchProducts(WebDriver driver, String searchQuery) {
        ...
        try {
            ...
            WebElement searchBox = wait.until(ExpectedConditions.elementToBeClickable(By.name("searchVal")));
            searchBox.clear();
            ...
            List<WebElement> productElements = driver.findElements(By.className("item"));
            
            for (int i = 0; i < Math.min(productElements.size(), 10); i++) {
                WebElement element = productElements.get(i);
                
                try {
                    String name = getTextSafely(element, ".nameCls, .brand, .name");
                    String price = getTextSafely(element, ".price, .price-value");
                    String rating = getTextSafely(element, ".rating");
                    String imageUrl = getAttributeSafely(element, "img", "src");
                    String productUrl = getAttributeSafely(element, "a", "href");
                    
                    if (!name.isEmpty()) {
                        products.add(new Product(name, price, productUrl, imageUrl, "Ajio", rating));
                    }
                } catch (Exception e) {
                    continue;
                }
            ...
            System.err.println("Error scraping Ajio: " + e.getMessage());
        }
